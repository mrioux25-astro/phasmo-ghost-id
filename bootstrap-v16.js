...
(() => {
  const VERSION="v16";
  const BUILD="Top Ghost Reference + Behavioral Strategy";
  function addVersion(){
  ...
  addVersion();
  load("./gameplay-v11.js?v=16","gameplayV11");
  load("./gameplay-v13.js?v=16","gameplayV13");
  load("./speed-language-v14.js?v=16","speedV14");
  load("./investigation-v15.js?v=16","investigationV15");
  load("./top-ghosts-v16.js?v=16","topGhostsV16");
  document.documentElement.dataset.ghostBuild=VERSION;
  if("serviceWorker" in navigator){
    navigator.serviceWorker.register("./sw.js?v=16",{updateViaCache:"none"}).then(r=>r.update()).catch(()=>{});
    let reloaded=false;
    navigator.serviceWorker.addEventListener("controllerchange",()=>{if(reloaded||sessionStorage.getItem("ghost-v16-reload"))return;reloaded=true;sessionStorage.setItem("ghost-v16-reload","1");location.reload();});
  }
})();